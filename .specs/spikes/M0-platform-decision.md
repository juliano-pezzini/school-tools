...
- ✅ **NFe (DANFE) — caminho grátis:** **`consultadanfe.com` `POST /api/v1/consulta`** (`{chave}`) retorna **`xml_base64` com itens+valores** + PDF — **sem cadastro/API-Key**, 60 req/min. ⚠️ **CORS restrito à origem deles**: chamada direta do navegador (ex.: github.io) recebe **403 "Origem não autorizada"** → **precisa de backend** (testado: sem header Origin, via servidor, **funciona** e não há Cloudflare — `UrlFetchApp` serve). Limite: **modelo 55** e **mês corrente** (mês anterior até dia 15); nota antiga → enviar XML em `/api/v1/danfe`. **cDV (mod-11) validável offline** — evita gastar consulta com leitura errada.
