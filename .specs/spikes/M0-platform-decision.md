...
- ✅ **NFe (DANFE) — caminho grátis:** **`consultadanfe.com` `POST /api/v1/consulta`** (`{chave}`) retorna **`xml_base64` com itens+valores** + PDF — **sem cadastro/API-Key**, 60 req/min. ⚠️ **CORS restrito à origem deles**: chamada direta do navegador (ex.: github.io) recebe **403 "Origem não autorizada"** → **precisa de backend**. ✅ **VALIDADO ponta a ponta (2026-06-21):** proxy **Apps Script** (`doGet ?chave` + `UrlFetchApp`, deploy "Qualquer pessoa", `backend/nfe-proxy.gs`) → `docs/nota/?proxy=<exec>` renderizou fornecedor+itens+total de uma NFe real (R$173,91). Front usa **GET ?chave** (sem preflight CORS, sem redirect de POST). Limite: **modelo 55** e **mês corrente** (mês anterior até dia 15); nota antiga → `/api/v1/danfe` com XML. **cDV (mod-11) validável offline** — já pegou um mis-scan no teste.
- 🟡 **NFe — fallbacks pagos:** `danferapida.com.br` (R$0,05/consulta + API-Key → backend; 100/dia grátis em outros endpoints) e **MeuDanfe** (R$0,03 + key + CORS → backend). Úteis para **notas fora do mês corrente**.
- ✅ **NFC-e (QR): CONFIRMADO com cupom real (2026-06-21, chave `4226068326...247765`).** O **link do QR** na SEFAZ-SC abre a nota completa (**itens + valor**) **sem captcha** — passa apenas por um **desafio Cloudflare**, depois mostra o resultado. ⚠️ O Cloudflare **bloqueia `UrlFetchApp`/fetch simples** → leitura automática dos itens precisa de **navegador real/headless** (não server-fetch puro). No app: abrir o link funciona; raspar os itens automaticamente é o ponto a resolver.
- 🟡 **OCR do cupom/DANFE impresso** (itens+total em texto) como fallback certificado-free quando QR/chave falham.
- 🔴 **Caminho oficial descartado por escolha:** e-CNPJ A1 + Distribuição DFe daria XML oficial, mas exige certificado pago — evitado. **Confirmado (2026-06-21):** portais SEFAZ/nacional só dão **resumo (valor total, sem itens)** + reCAPTCHA sem certificado; SEFAZ-RS agora exige login gov.br; NFe 55 **não tem QR** (exclusivo da NFC-e 65). Logo, itens de NFe 55 só via **scraping de terceiros** (consultadanfe/danferapida/MeuDanfe) ou XML.
