...
- ✅ **NFe (DANFE) — caminho grátis no navegador:** **`consultadanfe.com` `POST /api/v1/consulta`** (`{chave}`) retorna **`xml_base64` com itens+valores** + PDF — **sem cadastro, sem API-Key, CORS liberado** (chama direto do front), 60 req/min. ⚠️ Só **modelo 55** e **mês corrente** (mês anterior até dia 15); nota antiga exige enviar o XML em `/api/v1/danfe`. **Integrado e testando em `docs/nota/`** (2026-06-21).
