...
- ⚠️ **NFe (DANFE):** chave do código de barras → **MeuDanfe** por chave busca direto na Receita e retorna **XML completo (itens+valores)**, mas é **pago R$ 0,03/consulta + exige API-Key (cadastro)** e provável **bloqueio de CORS** → só via **backend/proxy** (a key não pode ir ao navegador). (Doc confirmada 2026-06-21; só a conversão XML→PDF é grátis.)
